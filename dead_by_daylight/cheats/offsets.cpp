...
bool cheats::offsets_t::get()
{
    auto from_pattern = [this]( std::string_view pat, std::uint16_t byte_offset ) {
        ...
        if ( !address ) {
            throw std::runtime_error( fmt::format( xorstr_("error looking for pattern \"{}\""), pat ) );
        }
        ...
        if ( !m_target_process.read( address, rip ) ) {
            throw std::runtime_error( fmt::format( xorstr_("pattern \"{}\" couldn't be read"), pat ) );
        }

        return ( address + rip + 4 ) - m_target_process.base_address();
    };

    uworld = from_pattern( xorstr_("48 8B 1D ?? ?? ?? ?? 48 85 DB 74 3B 41"), 3 );
    gnames = from_pattern( xorstr_("48 8B 05 ? ? ? ? 48 85 C0 75 5F"), 3 );
